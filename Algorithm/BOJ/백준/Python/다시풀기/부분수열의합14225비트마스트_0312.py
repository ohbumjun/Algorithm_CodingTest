...
# 1 << n : 1000
# 0001, 0011,  .. . . 1111
# 0001 : 0번째 idx만 포함되어 있음 ( 맨 오른쪽 )
# 0011 : 0번째, 1번째 idx 포함되어 있음
# 1011 : 0,1, 3번째 idx가 포함되어 있음

# 1111 & 1011 --> 1011 : 부분수열 --> 원소들의 조합 --> 0,1,3번째 idx

for i in range(1 << n):  # for(int i = 0001 ; i < 1111; i++)
    s = 0
    for j in range(n):
        # ex) 현재 i 조합은 0001 인데, 1 << j, 즉, 맨오른쪽 1 값이 존재하느냐
        # 즉, 배열 기준, 맨 오른쪽 값이 현재 부분수열에 포함되어 있느냐
        # ex) j == 0 : --> 배열의 0번째 idx --> 0001
        # ex) j == 1 : fron 0001 ---> to 0010
        if (i & (1 << j)):  # ex) i : 0001 / j : 0 --> 1 << j : 0001  ==> i & j == 0001 && 0001 == 0번째 idx만 포함된 상황
            s += a[j]
    c[s] = True
i = 1
