...
def simulate(a, k, x, y):
    ...
    # 다시 한번, 즉, 우리는 지금 구멍에 빠졌는지 안빠졌는지를 확인하는 방법이
    # 빨강, 파랑 공 각각을 우선 이동시켜놓고
    # 그 다음에 비로소 빠졌는지, 안빠졌는지를 검사한다
    # 한번의 path가 있다고 해보자
    # 한번의 기울임 에서 빨간공이 빠졌고, 파랑은 안빠졌다고 해보자
    # 하지만, 이후 path, 즉, 다른 기울임 형태에서 파랑이 빠질수도 있는 것이다
    # 예를 들어, 아래로 기울였더니, 빨강 빠짐, 빨강이 왼쪽측, 파랑이 오른쪽 측
    # 이후, 왼쪽으로 기울이면, 파랑이 같은 구멍으로 빠질수도 있다
    # 그래서, 다시 한번, 빨강, 파랑 모두에 대해 왼쪽으로 기울이는 경우도
    # 고려해서 처리해주어야 한다는 것이다
    if a[x][y] == '.':
    ...
    while True:
        ...
        # 오직 이 경우에만 return !
            ...
            # 왜냐면 구슬은 구멍에 해당하는 nx,ny에 빠졌고
            # 기존의 구슬은 이동처리가 되었으니, 기존 위치에서 구슬은 사라져야 한다
