...
def go( arr, idx, cur,  plus, minus, mul, div ) :
    ...
    res = []
    if plus > 0 :
        res.append(go( arr, idx + 1, cur + arr[idx],  plus - 1, minus, mul, div ))
    if minus > 0 :
        res.append(go( arr, idx + 1, cur - arr[idx],  plus, minus - 1, mul, div ))
    if mul > 0 :
        res.append(go( arr, idx + 1, cur * arr[idx],  plus, minus, mul - 1, div ))
    if div > 0 :
        if cur >= 0 :
            res.append(go( arr, idx + 1, cur // arr[idx ], plus, minus, mul, div -1 ))
        else :
            res.append(go( arr, idx + 1, - ( cur // arr[idx ] ), plus, minus, mul, div -1 ))
