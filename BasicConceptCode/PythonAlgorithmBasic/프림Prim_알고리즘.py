...
def prim(W):
    ...
    n = len(W) - 1
    F = []
    # 0번째 idx는 비어있는 상태로 작업 --> nearest[i] : i정점과 가장 가까운 정점 ex) 1번정점 --> 2번정점
    nearest = [-1] * (n + 1)
    distance = [-1] * (n + 1)  # i ~ nearest[i] : 걔네 거리

    # 정점을 선택해간다 : V = 전체 정점 --> (선택해가는 정점) vs (그외)
    # ( 1 , 2 ) -> ( 3, 4, 5, 6 )
    # 1 --- 3 : 5
    # 2 --- 3 : 3 ( 거리, 가장가까운 정점 )
    # 1 --> 2
    ...
    for i in range(2, n + 1):
        nearest[i] = 1  # 1 번 정점 --> start
        distance[i] = W[1][i]

    for _ in range(n-1):  # n 개의 노드(정점)가 선택될 때까지 반복
        minVal = INF
        for i in range(2, n + 1):
            '''
            Y에 선택되지 않은 노드 중에서,
